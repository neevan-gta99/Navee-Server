...
const womensEthnicSchema = new mongoose.Schema({
...
womensEthnicSchema.pre("save", async function (next) {
  ...
  if (!this.sku) {
    this.sku = SKU_Genertor.generateSKUForWomenEthnic(
      this.productID,
      this.name,
      this.material,
      this.fit
...
export const womensEthnicDTO = mongoose.models.womensEthnic || mongoose.model("womensEthnic", womensEthnicSchema);
